...
Data provider names must match test names.
...
Promoted properties in the constructor must be on separate lines.
Configuration options:
...
There must be no imports without namespace.
```diff
 <?php
 namespace Foo;
-use DateTime;
...
Classes without namespace cannot contain leading slashes.
```diff
 <?php
-$x = new \Foo();
+$x = new Foo();
...
There must be no parameters passed by reference in function.
...
Unnecessary `dirname` calls must be removed.
```diff
 <?php
...
`strlen` and `mb_strlen` functions must not be compared to 0.
...
The `@param` annotations must be in the same order as function parameters.
...
Promoted properties must read-only.
